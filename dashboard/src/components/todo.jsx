...
export default function Todo() {
  ...
  return (
    <div className='w-full min-h-screen flex flex-col items-center bg-gray-900 text-white'>
        <div className='border rounded-2xl m-30 w-120 shadow-gray-600'>
        ...
        <div className='text-center m-8 shadow-gray-600'>
            ...
            <button className='bg-blue-500 px-4 py-4 rounded-lg w-full ' onClick={Add}>Add</button>
        </div>
        <div className=' h-auto px-4 pb-4'>
        <ul className='w-full max-w-md space-y-3'>
            {tasks.map((task,index)=>
                <li key={index} className='bg-gray-800 rounded-lg p-3 flex justify-between items-center cursor-grab'draggable onDragStart={(e)=> onDragStart(e, index)} onDrop={()=>dropItem (index)} onDragOver={(e)=> e.preventDefault()}>
                <span>{task}</span>
                <button className='border text-red-200 bg-red-500 ' onClick={() =>Delete(index)}>Delete</button>
            </li>
            )}
        </ul>
        </div>
